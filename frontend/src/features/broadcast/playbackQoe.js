...
export function createQoeTracker({ now = () => performance.now() } = {}) {
  ...
  function accrue() {
    ...
    if (elapsed > 0) {
      ...
      else if (mode === STALLED) stallMs += elapsed
    }
    accountedAt = t
  }
  ...
  function seeking() {
    ...
    if (mode === PLAYING || mode === STALLED) activeBeforeSeek = true
    accrue()
    mode = PAUSED
    inSeek = true
  }
   ...
   * 그래서 이 함수는 hlsPlayer 의 fatal 분기에서만 불린다.
   */
  function error() {
    errors += 1
  }
  ...
  return { attached, playing, waiting, paused, seeking, resumedBySeekEnd, error, snapshot }
}
