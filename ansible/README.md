...
**공개 도메인은 vault 에 넣지 않는다.** (#186)
프론트 주소(`front_url`·`front_url2`)는 `vars.yml` 에 평문으로 둔다 —
비밀이 아니고, **vault 안에 있으면 코드 검토에서 안 보인다.**
실제로 개발용 `localhost` 가 운영에 그대로 남아 있었고 아무도 못 봤다.
그 값은 "브라우저가 붙을 수 있는 출처" 를 정하므로, 틀리면 부하 도구와
헬스체크는 다 통과하는데 진짜 브라우저만 403 을 받는다.
