...
|           |        [프로젝트 일정]        |
| :-------: |:-----------------------:|
| 진행 기간 | 2025.07.07 - 2025.08.18 |
|   인원    |           6명            |

## 일정과 작업 방식

진행 기간은 기획과 설계를 포함한 팀 프로젝트 전체 기간입니다. 1~2주차는 기획과 설계 기간이라 저장소 커밋이 없고 개발 커밋은 2025-07-22부터 2025-08-18까지 남아 있습니다.

### 팀 개발

`docs/team-rules.md`에 스크럼과 코드 리뷰 순서를 먼저 정해뒀습니다. 매일 15분 데일리 스크럼, 코드 리뷰는 설계 → 기능 → 복잡도 → 테스트 → 네이밍 → 문서 순서로 봤습니다. 브랜치 이름은 `<type>/<이슈번호>-<설명>` 형식을 쓰기로 정했습니다.

실제 커밋은 이 형식을 부분적으로 따랐습니다. 팀 기간 커밋 467건 중 `type: 제목` 형식을 지킨 것은 140건이고, 나머지는 자유 형식 메시지이거나 `Merge branch 'css' into 'master'`처럼 GitLab 머지 요청을 그대로 남긴 것입니다.
...
이슈를 먼저 만들고 이슈 번호를 단 브랜치에서 작업한 뒤 PR로 병합하는 흐름을 썼습니다. 2026년 8월부터 지금까지 이슈 69건, PR 124건을 열었고 그중 121건을 병합했습니다. 브랜치 이름은 `<type>/<이슈번호>-<설명>` 형식을 예외 없이 지켰습니다.
...
`perf` 커밋에 전후 측정치를 붙이는 규칙은 팀 기간과 동일하게 지켰습니다.
