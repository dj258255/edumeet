...
이 절만 **십진 GB 로 계산한다.** OCI 청구가 십진이고, 여기서 묻는 것이
*"언제 청구가 시작되나"* 이기 때문이다. (앞 절들의 표는 10절 규약대로 GiB 다.)

```
우리 실측(1.42 Mbps)   10,000 GB ÷ 0.639 GB/h  =  약 15,600 시청자-시간
720p (3 Mbps) 가정     10,000 GB ÷ 1.35  GB/h  =  약  7,400 시청자-시간
```

> **처음에는 위 두 줄이 단위가 달랐다.** 앞줄은 10,240 GiB, 뒷줄은 10,000 GB 로 나눠
> 실측 쪽이 16,400 으로 나왔다. 나란히 놓고 읽으면 **배율 차이의 일부가 비트레이트가 아니라
> 단위에서 온다.** 십진으로 통일하고 그 사실을 여기 남긴다.
...
4위  egress 요금 (720p 가정 기준 시청자 7,400명 × 1시간 이상)
```
...
- **단위 규약** — 본 문서의 표는 **GiB(÷1024)** 기준이다. 클라우드 청구는 **십진 GB(÷1000)** 라
  실제 청구량은 표보다 **약 7% 많다**. 다만 **7절(무료 한도 소진)만은 십진으로 계산한다** —
  거기서 묻는 것이 청구 시점이라 청구 단위와 맞춰야 한다. 두 규약을 한 절 안에서 섞지 않는다 (#147).
