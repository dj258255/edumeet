...
## AI 서비스 — 파이프라인은 만들되 켜지 않았다 (#110)

`Dockerfile`·compose 서비스·빌드 잡을 전부 만들었지만 **자동으로 돌지 않는다.**

```
compose      profiles: [ai]                       기본 비활성
워크플로     workflow_dispatch + inputs.deploy_ai  수동 트리거 전용
```
...
| | |
|---|---|
| `/STT/{class_id}` 를 부르는 곳 | **저장소 어디에도 없다** |
| 파이썬이 자막을 백엔드로 보내는 코드 | **없다** (#106) |

**배포해도 아무도 안 부르고, 부를 수 있어도 자막이 안 나간다.**

> 돌지 않는 서비스를 자동 배포하는 것은 이 저장소에서 **일곱 번 겪은
> "만들었는데 동작하지 않는다"** 를 하나 더 만드는 일이다.
> 그리고 그때는 **CI 가 초록이라 더 안 보인다.**

### ★ 프로필이 꺼져 있어도 변수는 평가된다

`profiles: [ai]` 로 꺼 뒀는데 **평범한 배포가 죽었다.**

```
error while interpolating services.ai.image: required variable AI_IMAGE is missing a value
```

**Compose 는 프로필이 꺼진 서비스라도 변수 보간을 먼저 한다.**
프로필 필터링은 그 다음 단계다.

`${AI_IMAGE:?}` 는 *"안 주면 실패하게"* 라는 좋은 의도였는데,
**꺼진 서비스에까지 적용되니 정반대 결과**가 났다. `:-` 로 자리표시자를 준다.

> 자리표시자로 두면 "조용히 엉뚱한 것이 뜨지 않나" 싶지만 —
> 그 태그는 레지스트리에 없으므로 `--profile ai` 로 켤 때 **pull 에서 실패한다.**

### 프로필의 함정을 알고 쓴다

`profiles` 로 꺼 두는 방식은 **#83 에서 관측 스택이 한 번도 안 켜졌던 바로 그 원인**이다.
Compose 는 프로필이 꺼진 서비스를 **조용히 건너뛴다.**

그래서 이번에는 **의도적으로** 그렇게 하고, 문서와 compose 주석 양쪽에 적었다.
차이는 "몰라서 꺼져 있는 것" 과 "알고 꺼 둔 것" 이다.

### 켜기 전에 필요한 것

1. 트리거 경로 — 누가 `/STT/{class_id}` 를 부르나
2. 파이썬 → 백엔드 자막 전송 (STT 스트리밍 설계)
3. 유료 키(`OPENAI_API_KEY` · `CLOVA_SECRET_KEY`)를 vault 에
