...
> **⚠ 이 문서의 측정은 H2 인메모리에서 쿼리 "개수"만 센 것이다.**
> 구조가 고쳐졌다는 증거이지 빨라졌다는 증거가 아니다.
> 운영과 같은 MySQL 에서 지연시간·처리량까지 잰 결과는
> [03. MySQL 부하 측정](03-mysql-load-test.md)에 있다.
> 거기서 **쿼리 수가 처리량과 단조 관계가 아니라는 것**도 확인했다.
