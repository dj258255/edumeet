...
로컬 수치는 설계 판단용으로만 쓴다. 제출용 숫자는 운영과 분리해서 다시 쟀다.

| 확인 | 결과 | 의미 |
|---|---|---|
| OCI 2 OCPU aarch64 ffmpeg 벤치마크 | 리먹싱 0.084초, 재인코딩 5.456초, audio-only 0.245초 | HLS 를 만드는 CPU 비용 |
| 운영 HLS viewer k6 | 20 VU, 1분 hold, 실패 0 / 1,968 | 이미 만들어진 HLS 를 나눠 주는 delivery 경로 |

두 값을 섞지 않는다.

- ffmpeg 벤치마크는 **생성 비용**이다.
- k6 HLS viewer 는 **배포 비용**이다.
- 둘을 섞으면 CPU 병목인지, nginx 파일 서빙인지, 캐시/네트워크 문제인지 알 수 없다.
