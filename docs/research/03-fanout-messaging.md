...
### ⚠ 이 비교의 범위 — Slack 은 팀 채팅이다

Slack 수치를 **"라이브 채팅도 Slack 처럼 하면 된다"** 로 확장하면 안 된다. 성격이 다르다.

| | Slack형(팀 채팅) | 라이브 스트리밍형 |
|---|---|---|
| 방 크기 | 수십~수백 | **수천~수만** |
| 발신 | 소수가 자주 | **다수가 가끔** |
| fan-out | 낮음 | **극단적** |
| 읽기 | 과거 기록을 자주 조회 | **거의 실시간만** |
| 저장 이유 | 검색·스레드·컴플라이언스 | **다시보기뿐** |

**Slack 사례는 "MySQL 이 초당 30만 쓰기를 감당한다" 는 용량 근거로만 쓴다.**

라이브 채팅의 병목은 **DB 가 아니라 fan-out** 이다. 그건 [#43](../performance/07-chat-unbounded-queue-oom.md) 에서
직접 측정했다 — **fan-out 144.5배, 큐 107만, 84초 만에 OOM.**
DB 는 그 그림에 등장하지도 않았다.
