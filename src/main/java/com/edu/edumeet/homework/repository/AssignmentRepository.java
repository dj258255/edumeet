...
public interface AssignmentRepository extends JpaRepository<Assignment, Long> {
    ...
    @Query("SELECT a FROM Assignment a LEFT JOIN FETCH a.attachmentFiles WHERE a.id = :id AND a.deletedAt IS NULL")
    Optional<Assignment> findByIdWithAttachmentFiles(@Param("id") Long id);

    @Query("SELECT a FROM Assignment a LEFT JOIN FETCH a.studentSubmissionStatuses WHERE a.id = :id AND a.deletedAt IS NULL")
    Optional<Assignment> findByIdWithSubmissionStatuses(@Param("id") Long id);
